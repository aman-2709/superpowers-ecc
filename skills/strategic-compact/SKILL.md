...
1. **Tracks message count** — Increments a counter in session state on each stop
2. **Threshold detection** — Suggests compaction at a configurable threshold (default: 50 messages)
3. **Periodic reminders** — Reminds every 25 messages after threshold
...
- `COMPACT_THRESHOLD` — Messages before first suggestion (default: 50)
